        ...
        <div className="verify-page">
            ...
            <div className="verify-container">
                ...
                <div className="verify-search-box">
                    <input
                        type="text"
                        placeholder="Enter Campaign ID (e.g., a1b2c3d4-...)"
                        value={campaignId}
                    ...
                    <div className="verify-result">
                        <div className="verify-status-card">
                            <div className="verify-status-header">
                                <span className="verify-status-icon">{getStatusIcon(status.verification_status)}</span>
                                <div>
                                    <h2>{status.organization_name}</h2>
                                    <span className={`status-badge status-${status.verification_status}`}>
                                        {status.verification_status.charAt(0).toUpperCase() + status.verification_status.slice(1)}
                                    </span>
                                </div>
                            </div>
                            <p className="verify-status-desc">{getStatusDescription(status.verification_status)}</p>
                            
                            {status.verification_notes && (
                                <div className="verify-notes">
                                    <strong>Notes from reviewer:</strong>
                                    <p>{status.verification_notes}</p>
                                </div>
                            )}
                            ...
                            {status.scheduled_date && (
                                <div className="verify-schedule">
                                    ...
                                    <p>{new Date(status.scheduled_date).toLocaleString()}</p>
                                </div>
                            )}

                            <div className="verify-meta">
                                <span>Campaign ID: <code>{status.id.slice(0, 8)}...</code></span>
                                <span>Submitted: {new Date(status.created_at).toLocaleDateString()}</span>
                            </div>
                        </div>
